...
segment_camera_tool = {
    "name": "segment_camera",
    "description": "Highlights or colors a specific object/area in the camera feed using AI segmentation. Use when the user says 'markiere', 'highlight', 'color', 'zeig mir' about objects in the camera. Runs on the user's browser GPU. Use simple English nouns for the target.",
    "parameters": {
        "type": "OBJECT",
        "properties": {
            "target": {
            ...
            "color": {
                "type": "STRING",
                "description": "CSS color for the overlay (e.g., 'rgba(255,255,0,0.4)', 'rgba(255,0,0,0.3)'). Default: semi-transparent yellow."
            }
        },
        "required": ["target"]
    }
...
tools = [{'google_search': {}}, {"function_declarations": [generate_cad, run_web_agent, create_project_tool, switch_project_tool, list_projects_tool, list_smart_devices_tool, control_light_tool, discover_printers_tool, print_stl_tool, get_print_status_tool, iterate_cad_tool, ha_list_entities_tool, ha_control_tool, ha_get_state_tool, set_reminder_tool, list_reminders_tool, cancel_reminder_tool, show_camera_tool, stop_camera_tool, annotate_camera_tool, segment_camera_tool] + tools_list[0]['function_declarations'][1:]}]
                        ...
                        for fc in response.tool_call.function_calls:
                            if fc.name in ["generate_cad", "run_web_agent", "write_file", "read_directory", "read_file", "create_project", "switch_project", "list_projects", "list_smart_devices", "control_light", "discover_printers", "print_stl", "get_print_status", "iterate_cad", "ha_list_entities", "ha_control", "ha_get_state", "set_reminder", "list_reminders", "cancel_reminder", "show_camera", "stop_camera", "annotate_camera", "segment_camera"]:
                                ...
                                if fc.name == "generate_cad":
                                    ...
                                    result_str = f"Segmentation request sent for '{target}'. The user's browser will process it on their GPU."
                                    function_response = types.FunctionResponse(
                                        id=fc.id, name=fc.name, response={"result": result_str}
                                    )
                                    function_responses.append(function_response)
